...
const OrderSummaryBox = (): JSX.Element => {
  const { t, locale } = useLanguage();
  const totalAmount = useSelector<ICartRootState, number>(
    (state) => state.cart.totalAmount
  );
  const totalQuantity = useSelector<ICartRootState, number>(
    (state) => state.cart.totalQuantity
  );

  const handleOrder = () => {
    // Call the Paystack checkout API to initiate checkout
    // Replace the placeholder values with your actual values
    fetch('/paystack/checkout', {
      method: 'POST',
      headers: {
    ...
    <>
      {totalQuantity > 0 ? (
        <div className="flex-grow sticky bottom-0 left-0 right-0 md:top-36 shadow-lg bg-palette-card border-2 rounded-lg py-4 xl:py-12 px-4 xl:px-8 -mx-[1rem] md:mx-4 xl:mx-8 mt-2 w-[100vw] md:w-auto  md:min-w-[300px] md:max-w-[400px]">
          ...
          <button onClick={handleOrder} className="block bg-palette-primary md:mt-8 py-3 rounded-lg text-palette-side text-center shadow-lg">
            {t.order}
          </button>
        </div>
      ) : (
